        ...
        given()
            ...
            .apply(
                document(
                    ...
                    requestFields(
                        fieldWithPath("title").type(JsonFieldType.STRING).description("Todo 제목, null 가능"),
                        fieldWithPath("categoryId").type(JsonFieldType.NUMBER).description("카테고리 ID, null 가능").optional(),
                        fieldWithPath("scheduledDate").type(JsonFieldType.STRING).description("예약 날짜 (yyyy-MM-dd), null 가능"),
                        fieldWithPath("scheduledTime").type(JsonFieldType.STRING).description("예약 시간 (HH:mm 또는 HH:mm:ss), null 가능").optional(),
                        fieldWithPath("notificationTime").type(JsonFieldType.STRING).description("알림 시간 (yyyy-MM-dd'T'HH:mm:ss, ISO-8601), null 가능").optional(),
                        fieldWithPath("isDone").type(JsonFieldType.BOOLEAN).description("완료 여부, null 가능"),
                    ),
                    responseFields(
                        fieldWithPath("status").type(JsonFieldType.NUMBER).description("HTTP 상태 코드"),
                        fieldWithPath("message").type(JsonFieldType.STRING).description("응답 메시지"),
